...
export function NewDeliveryClient({ products, warehouses, deliveryCount }: NewDeliveryClientProps) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)

  const [formData, setFormData] = useState({
    reference: `OUT-${String(deliveryCount + 1).padStart(3, "0")}`,
    contact: "",
    warehouse_id: "",
    responsible_person: "",
  ...
  const handleSave = async (status: "Draft" | "Ready") => {
    ...
    try {
      ...
      await createDelivery(deliveryData, productItems)
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Create a new outgoing inventory delivery
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-card border-border">
            <CardHeader>
              <CardTitle className="text-foreground">Delivery Details</CardTitle>
              <CardDescription>Basic information about this delivery</CardDescription>
            </CardHeader>
            <CardContent>
              <FieldGroup>
                ...
                <Field>
                  <FieldLabel>Contact (Customer/Destination)</FieldLabel>
                  <Input
                    value={formData.contact}
                    onChange={(e) =>
                      setFormData({ ...formData, contact: e.target.value })
                    }
                    placeholder="Enter customer or destination name"
                    className="bg-secondary border-border"
                  />
                </Field>
                <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <SelectContent className="bg-popover border-border">
                          {products.map((product) => (
                            <SelectItem key={product.id} value={product.id.toString()}>
                              {product.name} (Available: {product.free_to_use})
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
        ...
        <div className="space-y-6">
          <Card className="bg-card border-border">
            <CardHeader>
              ...
              <CardDescription>Save or validate this delivery</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <Button variant="ghost" className="w-full" asChild>
                <Link href="/dashboard/delivery">Cancel</Link>
              </Button>
            </CardContent>
          </Card>
